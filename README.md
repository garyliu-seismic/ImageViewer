...
Chromecast / DLNA 电视 / 小米盒子等设备。原理是 VLC 的 renderer：本地仍负责解码和
控制（播放/暂停/进度条/倍速照常），只把画面输出重定向到投屏设备。

- 需要 VLC（python-vlc 3.0.20+，自带 renderer 发现）
- 支持格式取决于投屏设备（mp4 一般没问题；mkv/外挂字幕可能投不出去）
- vosk 实时字幕是本地叠加窗口，不会跟投；外挂 .srt/.ass 由 VLC 内置渲染，会随画面投出
- 投屏中切换视频会自动投到同一设备；再点一次按钮可取消投屏、恢复本地播放
